...
const checkCouponIfAlreadyExist = async (couponCode: string[], userId: string) => {
  const couponRef = await firebaseDb
    .collection('herCoupon')
...
const createMyHerCoupon = async (userId: string) => {
  ...
  if(!user) return null;
  const name = user.displayName || user.phoneNumber?.split('+91')[1].slice(6,10) || 'THANKS';
  const random5 = [name, ...Array(5).fill('').map(() => `${name}${Math.floor(Math.random() * 100)}`)]
